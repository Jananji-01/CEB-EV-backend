...
//         try {
//             // Generate a temporary idTag (not used for actual session)
//             String idTag = "PREP-" + idDevice + "-" + System.currentTimeMillis();
...
//     // In startCharging method, change to real WebSocket:
...
//                 idTag = "IDT-" + idDevice + "-" + System.currentTimeMillis();
//             }
...
// //     // REST endpoint to trigger heartbeat manually
// //     @PostMapping("/device/{idDevice}/heartbeat")
// //     public ResponseEntity<Map<String, Object>> sendHeartbeat(@PathVariable String idDevice) {
// //     try {
// //         boolean sent = ocppWebSocketService.sendHeartbeat(idDevice); // use idDevice here

// //         Map<String, Object> response = new HashMap<>();
// //         response.put("success", sent);
// //         response.put("message", sent ? "Heartbeat sent" : "Device not connected");
// //         response.put("idDevice", idDevice);
// //         response.put("timestamp", LocalDateTime.now().toString());
...
public class ChargingEVController {
    ...
    public ResponseEntity<Map<String, Object>> getDeviceDetails(@PathVariable String idDevice) {
        try {
            ...
            // Get valid IdTag info for this device
            List<IdTagInfo> tags = idTagInfoRepository.findByIdDevice(idDevice);
            LocalDateTime now = LocalDateTime.now();
            
            for (IdTagInfo tag : tags) {
                if (tag.getExpiryDate().isAfter(now) && "Accepted".equals(tag.getStatus())) {
                    details.put("validIdTag", tag.getIdTag());
                    details.put("idTagExpiry", tag.getExpiryDate().format(DateTimeFormatter.ISO_LOCAL_DATE_TIME));
                    break;
                }
            }
    ...
    // Endpoint to get valid IdTag for a device
    @GetMapping("/device/{idDevice}/idtag")
    public ResponseEntity<Map<String, Object>> getValidIdTag(@PathVariable String idDevice) {
        try {
            List<IdTagInfo> tags = idTagInfoRepository.findByIdDevice(idDevice);
            LocalDateTime now = LocalDateTime.now();
            ...
            if (validTag == null) {
                // Generate new tag with 6 hours expiry
                String idTag = generateIdTag(idDevice);
                validTag = new IdTagInfo();
                validTag.setIdDevice(idDevice);
                validTag.setIdTag(idTag);
                validTag.setStatus("Accepted");
                validTag.setExpiryDate(now.plusHours(6));
                validTag.setCreatedAt(now);
                idTagInfoRepository.save(validTag);
            }
            
            Map<String, Object> response = new HashMap<>();
            response.put("success", true);
            response.put("idDevice", idDevice);
            response.put("idTag", validTag.getIdTag());
            response.put("status", validTag.getStatus());
            response.put("expiryDate", validTag.getExpiryDate().format(DateTimeFormatter.ISO_LOCAL_DATE_TIME));
            response.put("createdAt", validTag.getCreatedAt().format(DateTimeFormatter.ISO_LOCAL_DATE_TIME));
            ...
            return ResponseEntity.internalServerError().body(Map.of(
                "success", false,
                "message", "Failed to get IdTag: " + e.getMessage()
            ));
        }
    }
}